...
FOR(i,0,n)FOR(j,0,n){if(tree1[i][j]=='X')V[i][j]=1;if(tree2[i][j]=='X')V[i*n][j*n]=1;}
bfs(V,
...
maxi=0;
vector<string> s1,s2;
FOR(i,0,n){string t;cin>>t;s1.pb(t);}
FOR(i,0,n){string t;cin>>t;s2.pb(t);}
int k=SI;
printf("%d\n",maximumCycles(s1,s2,k));
	return 0;
}
