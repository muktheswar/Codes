...
double rnd(float d) //for rounding values
{
 ...
 class TheNumberGameDiv2{ 
		...
		string a=str(A);
		string b=str(B);
		string rev_b=string(b.rbegin(),b.rend());
		if(sz(b)==1)rev_b=b;
		...
		int count=0;
		FOR(j,i,i+sz(b)){
		if(a[j]!=b[count]){f=1;break;}
		count++;
		}
		if(prev==0)
		if(f==0 && i!=0 && sz(a)-sz(b)+2<mo){mo=sz(a)-sz(b)+2;break;}
		else
		if(f==0 && sz(a)-sz(b)<mo){mo=sz(a)-sz(b);break;}
		}
		FOR(i,0,sz(a)-sz(rev_b)+1){
		int f=0;
		int count=0;
		FOR(j,i,i+sz(rev_b)){if(a[j]!=rev_b[count]){f=1;break;}
		count++;
		}
		if(f==0 && sz(a)-sz(rev_b)+1<mo){mo=sz(a)-sz(rev_b)+1;break;}
		}
		if(mo==INF)return -1;
		return mo;
            } 
...
// BEGIN CUT HERE
	...
	void test_case_5() { int Arg0 = 123; int Arg1 = 123; int Arg2 = 0; verify_case(5, Arg2, minimumMoves(Arg0, Arg1)); }
