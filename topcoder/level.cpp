...
int m=INF;
VI b(10);
FOR(i,0,n)FOR(j,0,sz(a[i]))b[(int)a[i][j]-48]++;
int x,y;
FOR(i,0,sz(b)){if(1){FOR(j,0,sz(b))if(1){int sum=0;FOR(k,0,sz(b)){if(k!=i && k!=j && 1){sum+=min(abs(i-k),abs(j-k))*b[k];}}if(sum<m && abs(i-j)<=1){m=sum;x=i;y=j;}}}}
return m;
}
int main(){
int n;
