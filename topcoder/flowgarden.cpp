...
bool func1(const VI &a,const VI &b){
return *(a.end()-1)>*(b.end()-1);
}
void getOrdering(VI& f,VI& s,VI &e){
int n=sz(f);
vector<pair<pair<int,int>,int> >v;
FOR(i,0,sz(f))v.pb(mp( mp(f[i],s[i]) ,e[i]));
sort(all(v),func);
...
ans.pb(t);
FOR(i,0,n){
if(v[ini].second<v[i].first.second){ini=i;
sort(all(ans[count]));
count++;
VI t;
ans.pb(t);
...
if(v[ini].second<=v[i].second)ini=i;
}
ans[count].pb(v[i].first.first);
}
sort(all(ans[count]));
sort(all(ans),func1);
count=0;
FOR(i,0,sz(ans))FOR(j,0,sz(ans[i])){
if(count<n-1)printf("%d ",ans[i][j]);
else printf("%d\n",ans[i][j]);
count++;
}
return ;
