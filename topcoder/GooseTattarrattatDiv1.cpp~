...
double rnd(float d) //for rounding values
{
 ...
 class GooseTattarrattatDiv1{ 
        ...
        int getmin(string S) 
            ...
            char s[60];
			FOR(i,0,sz(S))s[i]=S[i];
			s[sz(S)]=0;
int n=sz(S);
map<char,int> a;
map<char,vector<int> >b;
for(int i=0;s[i]!=0;i++){a[s[i]]++;(b[s[i]]).pb(i);}
int co=0;
for(int i=0;i<n/2;i++){
if(s[i]!=s[n-i-1]){
co+=min(a[s[i]],a[s[n-i-1]]);
if(a[s[i]]>a[s[n-i-1]]){
a[s[i]]+=a[s[n-i-1]]-1;
a[s[n-i-1]]=0;
VI v=b[s[n-i-1]];
VI::iterator it;
for(it=v.begin();it!=v.end();it++){s[*it]=s[i];if(*it!=n-i-1)b[s[i]].pb(*it);}
b[s[n-i-1]].clear();
}
else{
a[s[n-i-1]]+=a[s[i]]-1;
...
VI v=b[s[i]];
VI::iterator it;
for(it=v.begin();it!=v.end();it++){s[*it]=s[n-i-1];if(*it!=i)b[s[n-i-1]].pb(*it);}
b[s[i]].clear();
}
}
}
