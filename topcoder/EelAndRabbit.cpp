...
double rnd(float d) //for rounding values
{
 ...
 class EelAndRabbit{ 
        ...
        int getmax(vector <int> l, vector <int> t) 
			...
			int L=INF,R=-1;
			int L1=L,R1=R;
			VI temp(sz(l)),final(sz(l));
			VI nul(sz(l));
			FOR(i,0,sz(l)){times.insert(l[i]);times.insert(l[i]+t[i]);}
			FIT(it,times){int count=0;L=INF;R=-1;FOR(i,0,sz(l)){
			if(*it>=t[i] && *it<=(l[i]+t[i])){if(t[i]<=L)L=t[i];if(l[i]+t[i]>=R)R=l[i]+t[i];count++;temp[i]=1;}
			}
			if(count>max1){max1=count;L1=L,R1=R;final=temp;temp=nul;}
			}
			FIT(it,times){int count=0;FOR(i,0,sz(l)){if(*it>=t[i] && *it<=(l[i]+t[i]) && final[i]==0){count++;}}if(count>max2)max2=count;}
			return max1+max2;
