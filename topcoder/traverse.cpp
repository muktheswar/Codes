...
if(po==N-1){
if(i!=N-2 && i!=0){
if(!cpresent(ne,i)){ne.pb(i);
if(i==-1)cout<<po<<"\n";
dfs(i,cnt+1);}
}
}
if(po!=0 && po!=N-1 && i!=po-1 && i!=po+1){
if(!cpresent(ne,i)){ne.pb(i);
if(i==-1)cout<<po<<"\n";
dfs(i,cnt+1);}
}
...
int count(int N, vector <int>& pts){
VI perm(N);
FOR(i,0,sz(pts))perm[pts[i]]=1;
dfs(pts[sz(pts)]-1,sz(pts));
return moves;
}
int main(){
cin>>N;
...
cin>>n;
VI pts(n);
FOR(i,0,n){int t;cin>>t;t--;pts[i]=t;}
cout<<count( N, pts)<<"\n";
	return 0;
}
