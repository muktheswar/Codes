...
FOR(i,0,sz(a))FOR(j,0,sz(a[i]))if(a[i][j]=='1'){dis[i][j]=d[j][i]=1;}
FOR(i,0,n){dis[i][i]=0;
FOR(j,0,n)
FOR(k,0,n)dis[i][j]=min(dis[i][j],dis[i][k]+dis[k][j]);
}
int in=0,out=0;
FOR(i,0,n){
int count1=0;
...
vector<string> s(n);
FOR(i,0,n)cin>>s[i];
VI ans=specialLocations(s);
out_c(ans.begin(),ans.end());printf("\n");
	return 0;
}
