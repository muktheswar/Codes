...
string getContainerInfo(string& s, VI& a, int l){
...
store[counti].insert(mp(i,i+l-1));
}
map<int,int> many;
FOR(i,0,sz(a))many[a[i]]+=1;
FOR(i,0,sz(a)){
if(store.count(a[i])>0){
int x=-1,y=INF;
FIT(it,store[a[i]]){
int r1=it->first;
int r2=it->second;
x=max(x,r1);y=min(y,r2);
if(r1>r2)swap(r1,r2);
//cout<<r1<<" "<<r2<<"\n";
cout<<a[i]<<" "<<many[a[i]]<<" "<<sz(store[a[i]])<<"\n";
if(many[a[i]]<sz(store[a[i]])){FOR(j,r1,r2+1)if(ans[j]!='+')ans[j]='?';}
else{cout<<"hi:"<<a[i]<<"\n";FOR(j,r1,r2+1)ans[j]='+';}
}
FOR(j,x,y+1)ans[j]='+';
}
