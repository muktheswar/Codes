...
void initialise(int node,char* a,VI& M,int start,int end){
if(start>end)return;
if(start==end){M[node]=(a[start]=='(')?1:-1;return;}
int mid=start+(end-start)/2;
initialise(2*node+1,a,M,start,mid);
initialise(2*(node+1),a,M,mid+1,end);
if(M[2*node+1]==-INF)M[node]=M[2*(node+1)];
if(M[2*(node+1)]==-INF)M[node]=M[2*node+1];
M[node]=M[2*node+1]+M[2*(node+1)];
return ;
}

void update(int node,char* a,VI& M,int start,int end,int po){
if(start>end)return;
if(start==end && end==po){M[node]=(a[start]=='(')?1:-1;return;}
int mid=start+(end-start)/2;
if(po<=mid)update(2*node+1,a,M,start,mid,po);
else update(2*(node+1),a,M,mid+1,end,po);
if(M[2*node+1]==-INF){M[node]=M[2*(node+1)];return ;}
if(M[2*(node+1)]==-INF){M[node]=M[2*node+1];return;}
M[node]=M[2*node+1]+M[2*(node+1)];
return ;
}
...
initialise(0,a,M,0,n-1);
for(int i=0;i<2*n+1;i++)printf("%d ",M[i]);printf("\n");
int Q=SI;
while(Q--){
int m=SI;
if(!m){
