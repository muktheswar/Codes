...
int dfs(vector<vector<bool> >& grid,vector<string>& sig,int start){
stack<node> S;
vector<vector<bool> > visit(160,vector<bool>(160));
node t;
t.ch=sig[0][start];
t.words=1;
t.in=0;
...
if(n==0)break;
set<string> insig;
FOR(i,0,n){string s;cin>>s;insig.insert(s);}
vector<string> sig;
string ac;
while(1){
string w;
getline(cin,w);
if(w=="LAST CASE")break;
istringstream in(w);
string t;
in>>ac;
while(in>>t)if(insig.find(t)!=insig.end())sig.pb(t);
if(sz(sig)>sz(ac)){cout<<ac<<" is not a valid abbreviation\n";continue;}
vector<vector<bool> > grid(160,vector<bool>(160));
vector<string> sub;
FOR(i,0,sz(sig)){
string check=ac.substr(i);sub.pb(check);
FOR(j,0,sz(sig[i])){if(cpresent(check,sig[i][j]))grid[i][j]=true;
}
}
int sum=0;
N=sz(sig);
...
if(sum==0){cout<<ac<<" is not a valid abbreviation\n";}
else cout<<ac<<" can be formed in "<<sum<<" ways\n";
}
