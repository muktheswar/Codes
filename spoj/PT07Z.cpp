...
Q.pop();
FOR(i,0,sz(V[top])){
if(!visit[V[top][i]]){Q.push(V[top][i]);visit[V[top][i]]=true;dis[V[top][i]]=max(dis[V[top][i]],dis[top]+1);
if(dis[V[top][i]]>maxi){maxi=dis[V[top][i]];index=V[top][i];}}
}
}
memset(dis,0,sizeof(dis));
memset(visit,false,sizeof(visit));
...
FOR(i,0,sz(V[top])){
if(!visit[V[top][i]]){Q.push(V[top][i]);visit[V[top][i]]=true;dis[V[top][i]]=max(dis[V[top][i]],dis[top]+1);}
if(dis[V[top][i]]>maxi){maxi=dis[V[top][i]];}
}
}
printf("%d\n",maxi);
	return 0;
