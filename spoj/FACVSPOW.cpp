...
if(low<a || low>high)return -1;
if(low==high){
//if(low==4)cout<<"hi"<<" "<<pre[4]<<" "<<mul*low<<"\n";
if(pre[low]>low*mul)return low;
else return -1;
}
int mid=low+(high-low)/2;
//cout<<"mid:"<<mid<<"\n";
int po=-1;
if(pre[mid]>mid*mul){po=bsearch(a,low,mid-1);/*cout<<low<<" "<<high<<" "<<mid<<" "<<po<<"\n";*/}
if(po==-1){
if(pre[mid]>mid*mul)return mid;
else{
//cout<<low<<" "<<high<<" "<<mid<<"\n";
po=bsearch(a,mid+1,high);
//if(po==4)cout<<"hi1\n";
return po;
}
}
...
int main(){
for(int i=1;i<=2718275;i++)pre[i]=pre[i-1]+log2(i);
pre[0]=-1;
int T=SI;
while(T--){
int a=SI;
mul=log2(a);
int po=bsearch(a,a,2718275);
printf("%d\n",po);
}
	return 0;
}
