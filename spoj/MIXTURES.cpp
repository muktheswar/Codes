...
while(scanf("%d",&n)!=EOF){
VI v(n);
for(int i=0;i<n;i++)v[i]=SI;
int ans=0;
while(sz(v)>1){
int mini=INF;
int po=-1;
for(int i=0;i<sz(v)-1;i++)if((v[i]*v[i+1])<mini){mini=(v[i]*v[i+1]);po=i;}
VI t;
if(po!=-1){
ans+=mini;
if(mini==0)break;
...
for(int i=0;i<po;i++)t.pb(v[i]);
t.pb((v[po]+v[po+1])%100);
for(int i=po+2;i<sz(v);i++)t.pb(v[i]);
v=t;
}
printf("%d\n",ans);
}
