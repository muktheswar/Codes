...
int col1[5679];
char st[4];
FOR(i,1,1235){row[i]=i;row1[i]=i;}
FOR(i,1,5679){col[i]=i;col1[i]=i;}
while(scanf("%s",st)==1){
char ch=st[0];
if(ch!='W')scanf("%d%d",&x,&y);
else scanf("%d",&x);
if(ch=='R'){swap(row[x],row[y]);row1[row[x]]=x;row1[row[y]]=y;}
if(ch=='C'){swap(col[x],col[y]);col1[col[x]]=x;col1[col[y]]=y;}
if(ch=='Q')printf("%d\n",(row[x]-1)*5678+col[y]);
//if(ch=='W'){int r=(x%5678==0)?(x/5678):(x/5678+1);printf("%d %d\n",row[r],col[x-(r-1)*5678]);}
if(ch=='W'){int r=(x+MAXC-1)/MAXC;printf("%d %d\n",row1[r],col1[(x-1)%MAXC+1]);}
}
	return 0;
}
