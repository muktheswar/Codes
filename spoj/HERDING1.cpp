...
int visit=0,count=0;
while(visit<n*m){
if(b[visit/m][visit%m]==0){
int x=visit/m,y=visit%m;
int start=x,end=y;
while(x>=0 && y>=0 && x<n && y<m && b[x][y]==0 ){
b[x][y]=1;
if(a[x][y]=='N'){x-=1;if(a[x][y]=='S' && b[x][y]==1){b[x][y]=2;break;}continue;}
if(a[x][y]=='S'){x+=1;if(a[x][y]=='N' && b[x][y]==1){b[x][y]=2;break;}continue;}
if(a[x][y]=='E'){y+=1;if(a[x][y]=='W' && b[x][y]==1){b[x][y]=2;break;}continue;}
if(a[x][y]=='W'){y-=1;if(a[x][y]=='E' && b[x][y]==1){b[x][y]=2;break;}continue;}
}
if(b[x][y]==2 ||( start==x && end==y)){b[x][y]=2;count++;ans.insert(x*m+y);}
}
else visit++;
}
//FIT(it,ans)cout<<*it<<" ";cout<<"\n";
if(sz(ans)>0)printf("%d\n",sz(ans));
else printf("1\n");
	return 0;
}
