...
inline bool ls(pair<int,int>& a,pair<int,int>& b){return (a.first<b.first && a.second<b.second);}
inline bool gte(pair<int,int>& a,pair<int,int>& b){return (a.first>b.first && a.second>b.second)||(a.first==b.first && a.second==b.second);}
inline bool gt(pair<int,int>& a,pair<int,int>& b){return (a.first>b.first && a.second>b.second);}

int index(vector<pair<int,int> >&table,int r,pair<int,int>& key){
int l=-1;
while(r-l>1){
int mid=l+(r-l)/2;
if(ls(table[mid],key))l=mid;
else if(gte(table[mid],key))r=mid;
else if(table[mid].first==key.first){
if(table[mid].second>key.second)return mid;
else return -1;
}
else if(table[mid].second==key.second){
if(table[mid].first>key.first)return mid;
else return -1;
}
}
if((key.first==table[r].first && key.second<table[r].second) || (key.second==table[r].second && key.first<table[r].first))return -1;
return r;
}
...
int len=0;
FOR(i,1,n){
if(ls(a[i],table[0])||((a[i].first==table[0].first && a[i].second<table[0].second) || (a[i].second==table[0].second && a[i].first<table[0].first)))table[0]=a[i];
else if(gt(a[i],table[len]))table[++len]=a[i];
else {int in=index(table,len,a[i]);if(in!=-1)table[in]=a[i];else table[i]=a[i];}
}
return len+1;
}
int main(){
...
FOR(i,0,n){int x=SI,y=SI;a.pb(mp(x,y));}
printf("%d\n",lis(a));
	return 0;
}
