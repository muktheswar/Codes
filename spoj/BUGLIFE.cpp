...
while (s.empty() == false) {
	...
	if(color[top]==color[graph[top][i]]){f=1;break;}
	}
	else{visit[graph[top][i]]=1;color[graph[top][i]]=(color[top]+1)%2;s.push(graph[top][i]);}
	}
	if(f==1)break;
	s.pop();
	}
...
printf("Scenario #%d:\n",test);
if(f==1)printf("Suspicious bugs found!\n");
