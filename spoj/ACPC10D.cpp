...
vector<vector<int> > dis(n,vector<int>(3,0));
FOR(i,0,n)FOR(j,0,3)grid[i][j]=SI;
dis[0][0]=INF;
dis[0][1]=grid[0][1];
dis[0][2]=grid[0][1]+grid[0][2];
for(int i=1;i<n;i++){
for(int j=0;j<3;j++){
int mini=dis[i-1][j];
if(j-1>=0){
if(dis[i-1][j-1]<mini)mini=dis[i-1][j-1];
if(dis[i][j-1]<mini)mini=dis[i][j-1];
}
if(j+1<3){if(dis[i-1][j+1]<mini)mini=dis[i-1][j+1];}
dis[i][j]=grid[i][j]+mini;
}
}
printf("%d. %d\n",++T,dis[n-1][1]);
}
