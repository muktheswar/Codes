...
/*LL solve(string& s,int low,int high){
if(high<low)return 0;
if(high-low==0)return 1;
if(s[low]<'2' || (s[low]=='2' && s[low+1]<='6'))if(high-low==1)return 2;
if(s[low]>='2'){
if((s[low]=='2' && s[low+1]>'6') || s[low]>'2'){
return solve(s,low+1,high);
}
return solve(s,low+1,high)+solve(s,low+2,high);
}
else return solve(s,low+1,high)+solve(s,low+2,high);
}
*/
int main(){
while(1){
...
vector<LL> ans(n);
vector<bool> flag(n);
FOR(i,1,n)if(s[i]=='0'){flag[i]=flag[i-1]=true;}
ans[n-1]=1;
if(flag[n-2] || s[n-1]=='0' ||(s[n-2]>='2' && s[n-1]>'6')||s[n-2]>'2')ans[n-2]=1;
else ans[n-2]=2;
for(int i=n-3;i>=0;i--){
if(flag[i] || flag[i+1] || s[i]=='0' || s[i+1]=='0' ||(s[i]>='2' && s[i+1]>'6')||s[i]>'2')ans[i]=ans[i+1];
