...
int len;
int bsearch(VI& a,int key,int mini,int low,int high){
if(low>high)return mini;
int mid=low+(high-low)/2;
if(abs(key-a[mid])<mini)mini=abs(key-a[mid]);
int f=0;
if(mid-1>=0 && mid+1<=len){
if(abs(key-a[mid-1])<abs(key-a[mid+1])){if(abs(key-a[mid-1])<mini)mini=abs(key-a[mid-1]);return bsearch(a,key,mini,low,mid-1);}
else if(abs(key-a[mid-1])>abs(key-a[mid+1])){if(abs(key-a[mid+1])<mini)mini=abs(key-a[mid+1]);return bsearch(a,key,mini,mid+1,high);}
else {if(abs(key-a[mid-1])<mini){mini=abs(key-a[mid-1]);}return mini;}
}
else if(mid-1>=0){return bsearch(a,key,mini,low,mid-1);}
else{return bsearch(a,key,mini,mid+1,high);}
}
...
int mini=INF;
if(n<m){
FOR(i,0,n){len=m-1;int ans=bsearch(b,a[i],mini,0,m-1);if(ans<mini)mini=ans;}
}
else{
FOR(i,0,m){len=n-1;int ans=bsearch(a,b[i],mini,0,n-1);if(ans<mini)mini=ans;}
}
printf("%d\n",mini);
}
	return 0;
