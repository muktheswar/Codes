...
int data[510][2];
int M[10010];
/*int solve(int total,int cost){
if(total>weight)return INF;
if(total==weight){M[total]=min(M[total],cost);return M[total];}
if(M[total]!=INF)if(M[total]<cost)return M[total];
int mini=INF;
FOR(i,0,n)mini=min(mini,solve(total+data[i][1],cost+data[i][0]));
return M[total]=mini;
}
*/
int main(){
...
int e=SI,f=SI;
n=SI;
FOR(i,0,n)data[i][0]=SI,data[i][1]=SI;
FOR(i,0,f-e+1)M[i]=INF;
M[0]=0;
weight=f-e;
for(int i=1;i<=weight;i++){
for(int j=0;j<n;j++){
if(i-data[j][1]>=0){
M[i]=min(M[i],data[j][0]+M[i-data[j][1]]);
}
}
}
if(M[weight]>=INF)printf("This is impossible.\n");
