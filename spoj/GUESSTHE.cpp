...
num.pb(1);
if(s[0]=='N'){printf("-1\n");continue;}
FOR(i,0,sz(s))if(s[i]=='Y')div.pb(i+1);else nd.pb(i+1);
FOR(i,0,sz(div)){
LL n=div[i];
int len=sz(num);
while(n!=0){
if(num[len-1]==1){if(n!=1)num.pb(n);break;}
else{
if(n%num[len-1]==0)n/=num[len-1];len--;
}
}
}
FOR(i,0,sz(num))ans*=num[i];
int f=0;
FOR(i,0,sz(nd))if(ans%nd[i]==0){f=1;break;}
if(f==1)printf("-1\n");
else printf("%lld\n",ans);
}
return 0;
