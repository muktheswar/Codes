...
void bfs(vector<vector<bool> >&flag,vector<vector<int> >&dis,pair<int,int>&start){
...
if(flag[x-1][y])dis[x-1][y]=0;
else dis[x-1][y]=dis[x][y]+1;
pair<int,int> temp;temp=mp(x-1,y);
Q.push(temp);}
}
if(x+1<n){
if(dis[x+1][y]>dis[x][y]+1){
...
if(flag[x][y-1])dis[x][y-1]=0;
else dis[x][y-1]=dis[x][y]+1;
pair<int,int> temp;temp=mp(x,y-1);
Q.push(temp);}
}
if(y+1<m){
if(dis[x][y+1]>dis[x][y]+1){
...
FOR(i,0,n){
FOR(j,0,m){
char t;scanf("%c",&t);/*printf("|%c|",t);*/
if(t=='1'){flag[i][j]=true;if(f==0){f=1;start=mp(i,j);}}
else flag[i][j]=false;
}
getchar();
}
