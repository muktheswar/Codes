...
int counti=0;
void bfs(int x,int y,vector<string>&a,vector<vector<int> >&color,vector<vector<bool> >&visit){
queue<node> Q;
node start;
start.x=x;
start.y=y;
...
for(int i=-1;i<=1;i++){
for(int j=-1;j<=1;j++){
if(i!=j && top.x+i>=0 && top.x+i<n && top.y+j>=0 && top.y+j<m && a[top.x+i][top.y+j]=='X' && !visit[top.x+i][top.y+j]){
node temp;
temp.x=top.x+i;
temp.y=top.y+j;
temp.color=counti;
color[temp.x][temp.y]=counti;
Q.push(temp);
}
}
}
...
int n=SI,m=SI,s=SI;
vector<string> a;
FOR(i,0,n){string t;cin>>t;a.pb(t);}
double den=(n-s+1)*(m-s+1),sum=0;
VVI(color,n,m);
vector<vector<bool> > visit(n,vector<bool>(m));
FOR(i,0,n)FOR(j,0,m)if(a[i][j]=='.')visit[i][j]=true;
FOR(i,0,n)FOR(j,0,m)if(a[i][j]=='X' && !visit[i][j]){counti++;bfs(i,j,a,color,visit);}
for(int i=0;i<n-s+1;i++){
for(int j=0;j<m-s+1;j++){
set<int> num;
for(int k=0;k<s;k++){
if(color[i+k][j+k]!=0)num.insert(color[i+k][j+k]);
}
sum=sum+sz(num);
}
}
