...
#define SI ({long long int x;scanf("%lld",&x);x;})
...
inline int pow(int n ,int k){if(k==0)return 1; int ans=1;while(k>0){if(k&1)ans=(ans*n);k>>=1;n=(n*n);}return ans;}
LL mod;
vector<vector<int> >m3(31,vector<int>(31));
vector<vector<int> >m(31,vector<int>(31));
vector<vector<int> >iden(31,vector<int>(31));
VI f(31);
int n;
inline vector<vector<int> > mul(vector<vector<int> >& m1,vector<vector<int> >& m2,int r1,int c1,int c2){
for(int i=0;i<r1;i++){
for(int j=0;j<c2;j++){
int ans=0;
for(int k=0;k<c1;k++)ans=(ans%mod+((LL)(m1[i][k])*(LL)(m2[k][j]))%mod)%mod;
m3[i][j]=ans%mod;
}
}
return m3;
...
inline vector<vector<int> > power(int p){
if(p==0)return iden;
if(p==1)return m;
vector<vector<int> > ans=power(p/2);
ans=mul(ans,ans,n,n,n);
if(p%2)ans=mul(ans,m,n,n,n);
return ans;
}
...
if(l<n){printf("%d\n",l);continue;}
mod=pow(10,k);
for(int i=0;i<n;i++)for(int j=0;j<n;j++)m[i][j]=iden[i][j]=0;
for(int i=0;i<n;i++)f[i]=n-i-1;
for(int i=0;i<n;i++)m[0][i]=1;
iden[0][0]=1;
for(int i=1;i<n;i++){m[i][i-1]=1;iden[i][i]=1;}
m=power(l-(n-1));
int ans=0;
for(int i=0;i<n;i++)ans=(ans%mod+((LL)(f[i])*(LL)(m[0][i]))%mod)%mod;
printf("%d\n",ans);
}
	return 0;
}
