...
//cin>>s;
char s[100];
for(int i=1;i<=10;i++)s[i]=SC;
s[11]=0;
int m=SI;
VI ans;
int po=0;
int f=0;
for(int i=1;i<=10;i++)if(s[i]=='1'){ans.pb(i);}
if(sz(ans)<2 && sz(ans)<m){cout<<"NO\n";return 0;}
if(sz(ans)==m && m==1){cout<<"YES\n";cout<<ans[0]<<"\n";return 0;}
VI final;
int a=0,b=0;
for(int i=10;i>=1;i--){
if(s[i]=='1'){
...
if(i%2!=0)if(s[j]=='1'){a=i,b=j;}
}
}
}
}
if(a==0 || b==0 || a==b){cout<<"NO\n";return 0;}
if(a<b)swap(a,b);
int co=0;
cout<<"YES\n";
while(co<m){
...
if(ans[j]==final[i-1])continue;
if(i%2)
if(sum1==sum2+ans[j])continue;
else
if(sum2==sum1+ans[j])continue;
f=1;
final.pb(ans[j]);
break;
}
...
if(sz(final)==m){
cout<<"YES\n";
FOR(i,0,sz(final))cout<<final[i]<<" ";cout<<"\n";
}
else cout<<"NO\n";*/
	return 0;
}
