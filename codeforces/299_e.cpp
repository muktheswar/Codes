...
vector<LL> v;
int count=1;
for(LL i=1;i<17715;i++){if(n%i==0)v.pb(i);if(i>n)break;}
cout<<"sz:"<<sz(v)<<"\n";
for(LL a=1;a<sz(v);a++)for(LL b=1;b<sz(v);b++)for(LL c=1;c<sz(v);c++)if(v[a]*v[a]*(v[b]+v[c])+v[b]*v[b]*(v[a]+v[c])+v[c]*v[c]*(v[a]+v[b])==n)count++;
cout<<count<<"\n";
}
	return 0;
}
