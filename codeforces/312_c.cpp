...
int dist(pair<int,int>&a,pair<int,int>&b){return sqrt((a.first-b.first)*(a.first-b.first)+(a.second-b.second)*(a.second-b.second));}
int main(){
int n,k;
n=SI;
k=SI;
...
FOR(i,0,n)FOR(j,0,n)a.pb(mp(i,j));
int d=INF;       //here INF is a number big enough
int	tot=0;
int f=0;
FOR(i,0,n){
FOR(j,i+1,n){
		++tot;
		if(tot>k){f=1;break;}
			if (abs(a[j].first-a[i].first)>=d)break;    //notice that "break" is only to be
