...
#define IATOV(a) ({vector<LL> v(a,a+sizeof(a)/sizeof(LL));v;})
#define CATOV(a) ({vector<char> v(a,a+sizeof(a)/sizeof(char));v;})
#define sieve(a) ({LL b=ceil(sqrt(a));VI d(a,0);VI e;LL f=2;e.pb(2);e.pb(3);for(LL x=1;x<b+1;x++){for(LL y=1;y<b+1;y++){LL n=(4*x*x)+(y*y);if(n<=a&&(n%12==1||n%12==5)){d[n]^=1;}n=(3*x*x)+(y*y);if(n<=a&&n%12==7){d[n]^=1;}n=(3*x*x)-(y*y);if(x>y&&n<=a&&n%12==11){d[n]^=1;}}}for(LL r=5;r<b+1;r++){if(d[r]){for(LL i=r*r;i<a;i+=(r*r)){d[i]=0;}}}for(LL c=5;c<a;c++){if(d[c]){e.pb(c);}}e;})
...
template<class T> inline vector<pair<T,LL> > FACTORISE(T n){vector<pair<T,LL> >R;for (T i=2;n>1;){if (n%i==0){LL C=0;for (;n%i==0;C++,n/=i);R.push_back(make_pair(i,C));}i++;if (i>n/i) i=n;}if (n>1) R.push_back(make_pair(n,1));return R;}
template<class T> inline T TOTIENT(T n) {vector<pair<T,LL> > R=FACTORISE(n);T r=n;for (LL i=0;i<R.size();i++)r=r/R[i].first*(R[i].first-1);return r;}
