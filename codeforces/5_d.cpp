...
int main(){
	...
	float t1,t2,t3;
	if(v_max>w){
//	float s=fabs(w*w-2*a*d)/(4*a);
	//t1=sqrt((w*w+a*d*2)/2)/a;
	t1=sqrt((w*w/2+a*d))/a;
	t2=(sqrt((w*w/2+a*d))-w)/a;
	v=sqrt(w*w+2*a*(l-d));
	if(v<=v_max)t3=(v-w)/a;
	else{
		float s1=(v_max*v_max-w*w)/(2*a);
		t3=((v_max-w)*v_max+(l-s1-d)*a)/(v_max*a);
	}
	cout<<t1<<" "<<t2<<" "<<t3<<"\n";
	printf("%.5f\n",t1+t2+t3);
	}
	...
	if(v<=v_max)t3=(v)/a;
		...
		float s1=(v_max*v_max)/(2*a);
		t3=(fabs(v_max)*v_max+(l-s1)*a)/(v_max*a);
	}
	printf("%.5f\n",t3);
	}
	return 0;
