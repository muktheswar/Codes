...
set<int> s1,s2;
for(int i=0;i<8;i++){int x=SI,y=SI;s1.insert(x);s2.insert(y);a.pb(mp(x,y));}
if(sz(s1)!=3 || sz(s2)!=3){printf("ugly\n");return 0;}
sort(all(a));
VI v1(all(s1)),v2(all(s2));
int x2=v1[1];
int y2=v2[1];
int f=0;
if(cpresent(a,mp(x2,y2))){printf("ugly\n");return 0;}
for(set<int>::iterator it=s1.begin();it!=s1.end();it++){
for(set<int>::iterator jt=s2.begin();jt!=s2.end();jt++){
if(*it==x2 && *jt==y2)continue;
if(!cpresent(a,mp(*it,*jt))){f=1;break;}
}
}
if(f==1)printf("ugly\n");
