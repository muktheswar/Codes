...
vector<LL> qu(m+1);
FOR(i,0,m){int a,b;LL c;cin>>a>>b>>c; pair< pair<int,int>,LL > t;t.first.first=a;t.first.second=b;t.second=c;op.pb(t);}
FOR(i,0,k){int a,b;cin>>a>>b;
FOR(j,a,b+1)qu[j]++;
}
FOR(i,0,sz(qu)-1){op[i].second*=qu[i+1];}
//FOR(i,0,sz(op))cout<<op[i].second<<" ";cout<<"\n";
FOR(i,0,sz(op)){
FOR(j,op[i].first.first-1,op[i].first.second)ar[j]+=op[i].second;
}
//FOR(i,0,m+1)cout<<qu[i]<<" ";
//cout<<"\n";
FOR(i,0,n)cout<<ar[i]<<" ";
	return 0;
}
