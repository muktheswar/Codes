...
LL maxi=0;
LL su=0;
FOR(i,1,n+1){LL t;cin>>t;sum+=t;store.pb(t);}
sort(all(store));
FOR(i,0,n/4)swap(store[n-i-1],store[4*i]);
//FOR(i,0,sz(store))cout<<store[i]<<" ";cout<<"\n";
FOR(i,1,n+1){LL t=store[i-1];if(t>maxi)maxi=t;if(i%4==0){a[0].pb(maxi);su+=maxi;maxi=0;}}
ans[0]=su;
su=0;
for(int j=1;j<m;j++){
for(int i=0;i<sz(a[j-1]);i+=4){LL t=max(a[j-1][i],max(a[j-1][i+1],max(a[j-1][i+2],a[j-1][i+3])));ans[j]+=t;a[j].pb(t);}
}
FOR(i,0,m){sum+=ans[i];}
cout<<sum<<"\n";
return 0;
