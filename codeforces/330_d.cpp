...
int grid[1004][1004];
vector<vector<int> > dis(1004,vector<int>(1004,INF));
int ex,ey,sx,sy;
int main(){
int n=SI,m=SI;
map<pair<int,int>,int> ene;
vector<string> s(n);
FOR(i,0,n)cin>>s[i];
FOR(i,0,n){
FOR(j,0,m){
if(s[i][j]=='T')grid[i][j]=-1;
else if(s[i][j]=='E'){grid[i][j]=10;ex=i;ey=j;}
else if(s[i][j]=='S')sx=i,sy=j;
else {grid[i][j]=s[i][j]-48;ene[mp(i,j)]=grid[i][j];}
}
}
...
if(x-1>=0 && !visit[x-1][y] && grid[x-1][y]!=-1){visit[x-1][y]=true;dis[x-1][y]=min(dis[x-1][y],dis[x][y]+1);Q.push(mp(x-1,y));}
if(x+1<n && !visit[x+1][y]&& grid[x+1][y]!=-1){visit[x+1][y]=true;dis[x+1][y]=min(dis[x+1][y],dis[x][y]+1);Q.push(mp(x+1,y));}
if(y-1>=0 && !visit[x][y-1]&& grid[x][y-1]!=-1){visit[x][y-1]=true;dis[x][y-1]=min(dis[x][y-1],dis[x][y]+1);Q.push(mp(x,y-1));}
if(y+1<m && !visit[x][y+1]&& grid[x][y+1]!=-1){visit[x][y+1]=true;dis[x][y+1]=min(dis[x][y+1],dis[x][y]+1);Q.push(mp(x,y+1));}
}
int maxi=dis[sx][sy];
//FOR(i,0,n){FOR(j,0,m)cout<<dis[i][j]<<" ";cout<<"\n";}
int res=0;
FOR(i,0,n){
FOR(j,0,m){
if(!(i==sx && j==sy) && grid[i][j]>=0 && grid[i][j]<10 && dis[i][j]<=maxi){res+=grid[i][j];}
}
}
printf("%d\n",res);
	return 0;
