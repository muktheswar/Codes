...
int dataMul[100004],dataAdd[100004];
int n;
inline void internalUpdate(int at, int mul, int add) {
    while (at < n) {
        dataMul[at] += mul;
        dataAdd[at] += add;
        at |= (at + 1);
...
inline void update(int left, int right, int by) {
...
int main(){
n=SI;
FOR(i,0,n){int t=SI;dataMul[i]=dataAdd[i]=t;}
update(0,2,1);
FOR(i,0,n){cout<<dataMul[i]<<"\n";}
	return 0;
}
