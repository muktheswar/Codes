...
int ho=StringToNumber(h);
int mi=StringToNumber(m);
int rm=StringToNumber(string(h.rbegin(),h.rend()));
if(rm>mi && rm<60){
cout<<h<<":"<<string(h.rbegin(),h.rend())<<"\n";
}
else{
string t,rev;
do{
...
t=str(ho);
if(sz(t)==1)t="0"+t;
rev=string(t.rbegin(),t.rend());
rm=StringToNumber(rev);
}while(rm>=60);
cout<<t<<":"<<rev<<"\n";
}
